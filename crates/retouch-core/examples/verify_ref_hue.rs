...
fn main() {
    ...
    println!("=== 参考匹配色调迁移测试 ===");
    println!("原图 mean_h={:.1}°  ref mean_h={:.1}°", analyze(&purple).color.mean_h_deg, ref_m.color.mean_h_deg);
    ...
    let diff_after = (m_out.color.mean_h_deg - 30.0).abs().min(360.0 - (m_out.color.mean_h_deg - 30.0).abs());
    println!("匹配后 mean_h={:.1}°  距离参考: 前={:.1}° 后={:.1}°  护栏={}  强度={:.1}",
    ...
    println!("=== 一键中性力度测试 ===");
    println!("原图 mean_l={:.3} std_l={:.3} mean_c={:.3}", base.tone.mean_l, base.tone.std_l, base.color.mean_c);

    let (auto, _) = run_auto(&gray, 256, 4, 1.0);
    let m_auto = analyze(&DynamicImage::ImageRgb8(auto));
    println!("自动后 mean_l={:.3} std_l={:.3} mean_c={:.3}", m_auto.tone.mean_l, m_auto.tone.std_l, m_auto.color.mean_c);
    assert!(m_auto.tone.std_l > base.tone.std_l * 1.20, "一键中性应明显提升反差");
    println!("[通过] 一键中性力度足够\n");
    ...
    for (label, strength) in [("0.0", 0.0f32), ("0.5", 0.5), ("1.0", 1.0)] {
        ...
        let dist = (m.color.mean_h_deg - ref_m.color.mean_h_deg).abs()
            .min(360.0 - (m.color.mean_h_deg - ref_m.color.mean_h_deg).abs());
        println!("强度 {} -> mix={:.2} mean_h={:.1}° 距参考={:.1}°", label, strength, m.color.mean_h_deg, dist);
    }
}
